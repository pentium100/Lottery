...
public class LastTenStrategyAll extends BaseStrategy {
	...
	public List<OddPredict> predict2(Match m, MatchMouth mm,
		...
		if((!downCompanyFilter.contains(mm.getCompany().toString()))&&(!downCompanyFilter.equals("*"))){
			return result;
		}
		...
		while(rs.next()){
			...
			if(rs.getInt("ttlCount")==10&&rs.getInt("smallCount")>=(asiaWarningPct1/10)&&rs.getInt("smallCount")<=(asiaWarningPct2/10)){
				
				OddPredict op = createOddPredict(m, mm, null, "大小小");
			...
			if(rs.getInt("ttlCount")==10&&rs.getInt("bigCount")>=(asiaWarningPct1/10)&&rs.getInt("bigCount")<=(asiaWarningPct2/10)){
				
				OddPredict op = createOddPredict(m, mm, null, "大小大");

				op.setMatchCount(10);
				op.setPossibility(new Float(rs.getInt("bigCount")*10.0));
				op.setRemark("2C");
				result.add(op);
